...
def recursive_search_matching_files(directory, file_pattern, search_pattern):
    """
    Recursively searches for a pattern in files matching a given file pattern.
    """
    for root, _, files in os.walk(directory):
        for file in files:
            if wildcard_match(file, file_pattern):  # Only process files matching the file pattern
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python script.py <directory> <file_pattern> <search_pattern>", file=sys.stderr)
        sys.exit(1)

    dir_name = sys.argv[1]
    file_pattern = sys.argv[2]  # e.g., "*.txt"
    search_pattern = sys.argv[3]  # The pattern to search within files

    print(f"Searching for '{search_pattern}' in files matching '{file_pattern}' in directory '{dir_name}' recursively...")
    recursive_search_matching_files(dir_name, file_pattern, search_pattern)
